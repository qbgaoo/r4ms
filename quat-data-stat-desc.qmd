...
<a href="datasets/ex02-01.csv" download="ex02-01.csv" class="btn btn-success"> <i class="bi bi-database-fill-down"></i> Download data </a>
...
```{r}
bins = 10
breaks <- seq(min_max_hr, max_max_hr, length.out = bins + 1)
breaks
```
...
<button class="btn btn-primary" id="shiny-button"> Launch Shiny App </button>

```{=html}
<script>
// Launch the Shiny application when the button is clicked
document.getElementById("shiny-button").onclick = function() {
    Shiny.onInputChange('shiny_trigger', Math.random());
};
</script>
```
```{r}
