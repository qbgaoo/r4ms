...
gower_dist <- daisy(x, metric = "gower", stand = T)
...
```{r}
two_step_clustering <- function(data, n_pre_clusters = 5, n_final_clusters = 3, distance_method = "euclidean", seed = 123) {
  set.seed(seed)
  ...
  distance_matrix <- dist(data, method = distance_method)
  
  # Perform hierarchical clustering
  hclust_result <- hclust(distance_matrix, method = "ward.D2")
  
  # Create pre-clusters (cut the dendrogram into n_pre_clusters groups)
  pre_clusters <- cutree(hclust_result, k = n_pre_clusters)
  
  # Add the pre-cluster information to the dataset
  data_with_preclusters <- data
  data_with_preclusters$pre_cluster <- as.factor(pre_clusters)
  
  # STEP 2: Final Clustering using K-means
  # Now, apply k-means clustering on the pre-clustered data
  kmeans_result <- kmeans(data_with_preclusters, centers = n_final_clusters)
  
  # Add final cluster labels to the original data
  data_with_preclusters$final_cluster <- as.factor(kmeans_result$cluster)
  
  # Visualization: Final clusters (using the first two principal components)
  fviz_cluster(kmeans_result, data = data, geom = "point", ellipse.type = "convex", show.clust.cent = TRUE) +
    labs(title = "Two-Step Clustering Result") +
    theme_minimal()
  
  # Return the clustered data with pre-cluster and final cluster assignments
  return(data_with_preclusters)
}

# Example usage with the mtcars dataset
df <- mtcars
clustered_data <- two_step_clustering(df, n_pre_clusters = 5, n_final_clusters = 3)
...
Latent class analysis (LCA) is another method that can handle mixed data types, particularly when categorical variables dominate. It assumes that the data comes from a mixture of underlying classes or clusters and aims to estimate these latent (unobserved) classes.

It models the probability that an observation belongs to each latent class based on the observed variables. LCA is particularly suitable for categorical variables but can be extended for mixed data.

```{r}
library(poLCA)

# Create a formula for LCA with mixed variables
f <- cbind(age, gender, cholesterol) ~ 1  

# Perform latent class analysis with 3 classes
lc_result <- poLCA(f, data = x, nclass = 2, graphs = T)
```
...
X <- Byar |> 
  ...
  dplyr::select(1, 2, 5, 6, 8, 9, 10, 11, 3, 4, 12, 7) |>  # <2>
  mutate(
