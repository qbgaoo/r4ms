...
**例3-1** 若某市1999年18岁男生身高服从均数为167.7cm，标准差为5.3cm的正态分布。从该正态分布N(167.7, 5.3^2^)cm总体中随机抽样100次即共抽取样本g=100个，每次样本含量10人，计算得到每个样本的均数及标准差，数据以 `.csv` 格式保存，利用`read_csv()`函数导入R变量 `height_sample` 中。
...
因此，两组治疗前基线的IL-2总体均数之差的95%可信区间为`paste0("(", ci[1], ", ", ci[2], ")")`。
