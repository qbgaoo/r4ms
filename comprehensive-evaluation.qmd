...
# 构建成对比较矩阵
# 第一级：总目标
criteria <- c("Dietary", "Nursing", "Medical")
criteriaMatrix <- matrix(c(
  1,   1/3, 3,
  3,   1,   5,
  1/3, 1/5, 1
), nrow = 3, byrow = TRUE, dimnames = list(criteria, criteria))

# 第二级：子目标矩阵
# 膳食服务下的成对比较矩阵
dietaryMatrix <- matrix(c(
  1), nrow = 1, dimnames = list("Food Quality Rate", "Food Quality Rate"))

# 护理工作下的成对比较矩阵
nursingMatrix <- matrix(c(
  1), nrow = 1, dimnames = list("Execution Excellence Rate", "Execution Excellence Rate"))


# 医疗服务下的成对比较矩阵
subCriteria <- c("Medical System", "Medical Service Quality", "Bed Utilization")
medicalMatrix <- matrix(c(
  1,   3, 5,
  1/3, 1, 3,
  1/5, 1/3, 1
), nrow = 3, byrow = TRUE, dimnames = list(subCriteria, subCriteria))


# 第三级：具体评价指标矩阵
# 医疗质量下的成对比较矩阵
subCriteria <- c("ICU Admission Rate", "Clinical Efficacy Rate")
medicalServiceQualityMatrix <- matrix(c(
  1,   1/3,
  3,   1
), nrow = 2, byrow = TRUE, dimnames = list(subCriteria, subCriteria))
...
alternativesList <- list(
  FoodQuality = alternativesData$food_qulity,
  NursingExcellence = alternativesData$nurs_complic,
  MedicalExcellence = alternativesData$complic_excelnc,
  ICUAdmission = alternativesData$icu_admsin,
  ClinicalEfficacy = alternativesData$clinicl_eficacy,
  BedOccupancy = alternativesData$bed_ocupacy
)
