...
## Comparison of one-sample mean with a known population mean
...
## Comparison of one-sample proportion with a known population proportion
...
Alternative, you can use the `pwr.p.test()` function from the `pwr` packages:
...
```{r}
h <- ES.h(0.95, 0.8)
pwr.2p.test(h = h, sig.level = 0.05, power = 0.9, alternative = "two")
```
...
If you areworking with proportions that are close to 0 or 1, pwr.2p.test() may provide a more accurate sample size estimate due to Cohen’s h. If both proportions are more central, then either method could be appropriate, though results will vary due to these differences.

## Comparison of multiple sample proportions

When comparing the proportions of multiple samples, we are typically interested in determining whether there are statistically significant differences among the groups. Chi-square test is used to determine if there is a significant association between categorical variables. It can be applied when you have one categorical variable from two or more independent samples. The null hypothesis assumes that the proportions are equal across all groups.

To estimate the sample size required for detecting a difference among groups, the following formula can be used:

$$
n = \frac{\lambda} {2(\text {sin}^{-1} \sqrt{p_{\text max}} - \text {sin}^{-1} \sqrt{p_{\text min}})^2}
$$
...
```{r}
P <- matrix(c(0.4, 0.6, 0.5, 0.5, 0.65, 0.35), nrow = 3, byrow = T)
w = ES.w2(P / 3) 

pwr.chisq.test(w = w, df = 2, sig.level = 0.05, power = 0.9) 
```
