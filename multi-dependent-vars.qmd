...
```{r}
df <- read_csv("datasets/ex17-04.csv", show_col_types = F) |> 
  mutate(
    group = factor(group),
    group = relevel(group, ref = 2)
  )
```
...
You can directly use `summary.aov()` to extract the univariate ANOVAs for each dependent variable.

```{r}
summary.aov(model)
```

This will give you the individual ANOVA tables for each dependent variable. If any of these ANOVAs are significant, you should conduct post-hoc pairwise comparisons.
